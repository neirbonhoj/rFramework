    ...
    class VehicleManager : BaseScript
    {
        ...
        public VehicleManager()
        {
            ...
            RegisterCommand("echeckdamage", new Action<int, List<object>, string>((source, args, raw) =>
            {
                ...
                for (float i = StartX; i < FinalX; i += xIncrement / 2)
                {
                    for(float j = StartY - yIncrement; j < FinalY + yIncrement; j += yIncrement / 2)
                    {
            ...
            RegisterCommand("eapplydamage", new Action<int, List<object>, string>(async (source, args, raw) =>
            {
                ...
                foreach (Vector3 TestPoint in PointDeformation.Keys)
                {
                    if (PointDeformation[TestPoint].Length() > 0)
                    {
                        ...
                        SetVehicleDamage(Veh.Handle, CarPos.X + TestPoint.X, CarPos.Y + TestPoint.Y, CarPos.Z + TestPoint.Z, (float)Math.Pow(10, PointDeformation[TestPoint].Length() * 100), radius, true);
                    }
                }
        ...
        private async Task DrawRelativeVehiclePoint()
        {
            ...
            DrawBox(CarPos.X+-0.01f, 2.2f+CarPos.Y+-0.01f, CarPos.Z+-0.01f, CarPos.X+0.01f, 2.2f+CarPos.Y+0.01f, CarPos.Z+0.01f, 255, 255, 255, 255);
